        ...
        for (WaitNode q; (q = waiters) != null; ) {
            if (UNSAFE.compareAndSwapObject(this, waitersOffset, q, null)) {
                for (;;) {
                    ...
                    q = next;   // 循环遍历链表：处理完当前节点q后，将下一节点next，赋值给当前节点q，继续循环处理。
                }
                break;
            }
        }
    ...
    private int awaitDone(boolean timed, long nanos) {
        ...
        for (;;) {
            ...
            if (s > COMPLETING) { // 任务执行完成
                ...
                //  1、先把 waiters 賦值給 q.next，2、再把 q 賦值給 waiters，也就是变相初始化了 waiters属性值。
                // 當 waiters 屬性有初始值時，也就相當于多個線程調用___當第二個線程進來時，waiters 屬性是有值的，第二個線程變成了頭結點了。
